...
AS BEGIN TRAN
    ...
    IF @PREFERRED_OPTION = 1 AND EXISTS (SELECT CUSTOMER_CREDIT_CARD_NUMBER
        FROM CREDIT_CARD WHERE CUSTOMER_IDENTIFIER = @CUSTOMER_IDENTIFIER
...
CREATE OR ALTER PROC USP_CREDIT_CARD_UPD
...
AS BEGIN TRAN
    IF NOT EXISTS (SELECT CUSTOMER_CREDIT_CARD_NUMBER FROM CREDIT_CARD
        WHERE CUSTOMER_CREDIT_CARD_NUMBER = @CUSTOMER_CREDIT_CARD_NUMBER) BEGIN
    ...
    IF @PREFERRED_OPTION = 1 BEGIN
        UPDATE CREDIT_CARD SET PREFERRED_OPTION = 0
        WHERE CUSTOMER_IDENTIFIER = @CUSTOMER_IDENTIFIER
            AND PREFERRED_OPTION = 1
    ...
    UPDATE CREDIT_CARD
    SET CUSTOMER_CREDIT_CARD_NAME = @CUSTOMER_CREDIT_CARD_NAME,
